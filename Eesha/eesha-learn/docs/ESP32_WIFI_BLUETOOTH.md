...
Eesha Learn emula WiFi y Bluetooth (BLE) en el ESP32 usando el fork de QEMU de lcgamboa con soporte de red slirp. Cada instancia de emulación obtiene su propia red NAT aislada — ideal para múltiples usuarios simultáneos.
...
│  │    • "Eesha Learn-GUEST"  (ch 6, -20 dBm, open)                  │ │
...
- **SSID principal**: `Eesha Learn-GUEST` (canal 6, abierto, sin contraseña)
...
Cuando presionas "Run" en el editor, Eesha Learn escanea automáticamente tu código buscando patrones WiFi:
...
```
Sketch ejecuta WiFi.begin("Eesha Learn-GUEST", "")
         │
         ▼
...
WebSocket → Frontend: wifi_status event
         │
         ▼
QEMU UART0: "I (800) wifi:connected with Eesha Learn-GUEST, aid = 1"
         │
         ▼
Backend: { status: "connected", ssid: "Eesha Learn-GUEST" }
         │
         ▼
...
Cuando tu sketch ejecuta un WebServer en el ESP32, Eesha Learn crea un proxy HTTP reverso para que puedas acceder desde tu navegador:
...
Eesha Learn detecta el uso de BLE en tu sketch y muestra el estado de inicialización:
...
2. **Usa el SSID `Eesha Learn-GUEST`** (sin contraseña)
...
I (500) wifi:new:Eesha Learn-GUEST, old: , ASSOC
I (800) wifi:connected with Eesha Learn-GUEST, aid = 1, channel 6
...
const char* ssid = "Eesha Learn-GUEST";
const char* password = "";
...
void setup() {
  Serial.begin(115200);
  BLEDevice::init("Eesha Learn-ESP32");
...
Eesha Learn incluye 4 ejemplos pre-cargados accesibles desde la galería de ejemplos:
...
Conecta a `Eesha Learn-GUEST` y muestra la información de red.
...
void setup() {
  ...
  WiFi.begin("Eesha Learn-GUEST", "", 6);
  while (WiFi.status() != WL_CONNECTED) {
    delay(100);
    Serial.print(".");
  }
...
| Limitación | Detalle |
|------------|---------|
| **SSID fijo** | Debes usar `"Eesha Learn-GUEST"` (sin contraseña). No puedes crear tu propio AP ni usar otro SSID para conectar. |
...
| Archivo | Cambio |
|---------|--------|
| `third-party/qemu-lcgamboa/hw/misc/esp32_wifi_ap.c` | Añadido SSID "Eesha Learn-GUEST" al array de access points |
...
- El firmware debe usar el SSID `"Eesha Learn-GUEST"` — otros SSIDs no funcionarán
- No uses contraseña: `WiFi.begin("Eesha Learn-GUEST", "")`

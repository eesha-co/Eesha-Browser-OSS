...
This document describes how Eesha Learn emulates the Raspberry Pi Pico W's
...
| Does the on-board LED on Pico W work? | Yes. The LED is wired through the CYW43 chip on real hardware — Eesha Learn observes the `gpioout` IOCTL and toggles the canvas LED. |
| What SSID does it advertise? | `Eesha Learn-GUEST` (BSSID `02:42:DA:42:00:01`, channel 6). Same naming convention as the ESP32 path. |
| Is the closed firmware blob shipped? | **No.** Eesha Learn absorbs the 224 KB firmware-stream writes and synthesises the post-load chip state. See `docs/wiki/picow-cyw43-emulation.md`. |
...
| Sample SSID | `Eesha Learn-GUEST` | `Eesha Learn-GUEST` |
...
├── virtual-ap.ts                    Eesha Learn-GUEST single-source-of-truth
...
- **Bluetooth.** The CYW43439 also does BT 5.2; Eesha Learn doesn't model

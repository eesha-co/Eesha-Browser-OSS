# Sketch Engine — Eesha Browser's Own Search Engine
...
| Sketch Engine (Bun) | Browser-facing API | Real-time |
...
### 2. Start the Sketch Engine Service (Bun)

```bash
cd mini-services/sketch-engine
bun run dev
```
...
```bash
# Single run
python3 sketch-engine/scripts/seed_generator.py --once

# Continuous (every hour)
python3 sketch-engine/scripts/seed_generator.py
```

### 5. Set Up Cron Jobs

```bash
crontab sketch-engine/scripts/crontab
```
...
5. **Sketch Engine** (Bun service) wraps SearXNG with a clean API and beautiful UI for Eesha Browser
...
Sketch Engine v0.9.2 — Part of Eesha Browser

...
**Eesha Learn** — a fully local, open-source Arduino emulator 
...
- [backend/app/core/config.py](backend/app/core/config.py) - Settings (SECRET_KEY, DATABASE_URL `eesha-learn.db`, GOOGLE_*)
...
- `frontend/index.html` — Full SEO meta tags, OG, Twitter Card, JSON-LD. **Domain is `https://learn.eesha.onrender.com`** — update if domain changes.
...
For Eesha Learn-native components that don't exist in wokwi-elements (custom

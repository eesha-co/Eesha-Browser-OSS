# Eesha Learn MCP Server

Eesha Learn exposes a [Model Context Protocol](https://modelcontextprotocol.io/) (MCP) server that allows AI agents (e.g. Claude, Cursor) to:
...
| `import_wokwi_json` | Parse a Wokwi `diagram.json` → Eesha Learn circuit |
| `export_wokwi_json` | Serialise a Eesha Learn circuit → Wokwi `diagram.json` |
...
Eesha Learn circuits are plain JSON objects:

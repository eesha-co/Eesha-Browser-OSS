 ...
 *
 * Converts between Wokwi's diagram.json format and Eesha Learn's internal
...
export interface EeshaLearnComponent {
  id: string;
  metadataId: string;
  x: number;
  y: number;
...
export interface ImportResult {
  ...
  components: EeshaLearnComponent[];
  wires: Wire[];
...
// Wokwi board type → Eesha Learn boardType
...
// Eesha Learn boardType → Wokwi type
...
// Eesha Learn boardType → default Wokwi part id
...
export async function exportToWokwiZip(
  ...
  zip.file(
    'wokwi-project.txt',
    `Exported from Eesha Learn\n\nSimulate this project on https://learn.eesha.onrender.com\n`,
  );
...
export async function importFromWokwiZip(file: File): Promise<ImportResult> {
  ...
  // Eesha Learn internal component ID for the board element (must match DOM element id)
  ...
  // Convert non-board parts to Eesha Learn components.
  // Apply the same offset so components keep their relative position to the board.
  const components: EeshaLearnComponent[] = diagram.parts
  ...
  // Convert connections to Eesha Learn wires
  const wires: Wire[] = diagram.connections.map((conn, i) => {
    ...
    // Remap board part id → Eesha Learn internal board id
